...
/* constants -----------------------------------------------------------------*/
#define VER_RTKLIB "demo5 2.4.3" /* library version */
...
#define PMODE_TC    4        /* positioning mode: GNSS tight coupling*/
...
/* AR mode */
#define OFF     0       /* AR mode: off , for test instantaneous float solutions */
...
#define PAR_BIE 4       /* AR mode: PAR + BIE */
...
/* solution statistics output option ----------*/
...
#define RTKLIB_FILEPATHSEP '/'
    ...
    {                     /* outage simulation */   
        ...
        int valid_flag;   /* Is the current epoch within the set interruption period? (0:no, 1:yes) */
        int week;         /* GPS week */
    ...
    {                               /* earth related record  */
        double wie;
        double wnie[3];             /* angular velocity of earth's rotation in n frame */
        double wnen[3];             /* angular velocity of carrier motion in n frame */
        double wnin[3];             /* angular velocity of the n frame relative to the i frame */
        double Rl;                  /* earth's long radius */
        double alpha;               /* earth's oblateness */
        double Rs;                  /* earth's short radius */
        double e1;                  /* earth's first eccentricity */
        double e2;                  /* earth's second eccentricity */
        double RM;                  /* curvature radius corresponding to longitude */
        double RN;                  /* curvature radius corresponding to latitude */ 
        double g0;                  /* equatorial gravity */
        double g;                   /* local gravity */
        double beta[4];             /* gravity related parameters */
        double gn[3];               /* gravity vector in n frame */
        double wnien[3];            /* wnien=2*wnie+wnen*/
        double gcc[3];              /* harmful acceleration vector */
        double F1[9];                       
        ...
        double Frp[9];              /* error transformation matrix from ENU frame to BLH frame */
    } eth_t;
    ...
    {                                /* ins data record */
        ...
        double p1dw[3];              /* gyroscope angle increment of the previous epoch */
        double p1dv[3];              /* accelerometer speed increment of the previous epoch */            
        double pos[3];               /* ins position (B,L,H) (rad,rad,m) */
        double vel[3];               /* ins velocity in n frame (m/s) (E,N,U) */
        double body_vel[3];          /* ins velocity in b frame (m/s) */
        double nhc_vel[3];           /* ins velocity in v frame (m/s) */
        ...
        double *F;                   /* state transition matrix of continuous time system */
    ...
    {                            /* processing options type */
        ...
        int artype;              /* LAMBDA algrithm type (0,OFF,1:FAR,2:PAR,3:BIE,4:PAR-BIE)*/
        ...
        gtime_t ts;              /* start processing time */
        gtime_t te;              /* end   processing time */                    
    ...
    {                       /* solution options type */
        ...
        int satdop;         /* number of observed satellites and dop (0:off,1:on) */
    ...
    {                             /* file options type */
        ...
        char nav[MAXSTRPATH];     /* broadcast ephemeris file */
    ...
    {                              /* satellite status type */
        ...
        double cdtr[2];            /* receiver clock error (m) and clock drift (m/s)  */
    ...
    typedef struct
    {                               /* tight coupling option */
        int rfsat[NSYS][NFREQ];     /* reference satellite for each system of current epoch */
        int prfsat[NSYS][NFREQ];    /* reference satellite for each system of previous epoch */
        int arfsat[NFREQ];          /* reference satellite for all systems of current epoch */
        int parfsat[NFREQ];         /* reference satellite for all systems of previous epoch */
        int sflag[NSYS][NFREQ][2];     /* reset flag*/
    } tightc_t;    
    ...
    EXPORT extern outsim_t outsim;                         /* default outage simulation options */
    ...
    EXPORT int  isoutage(rtk_t *rtk, gtime_t t, outsim_t sim);
    ...
    EXPORT void pos2ecef2(const double *pos,double *r,double *T);
    ...
    EXPORT void covtosol(const double *P, sol_t *sol);
    EXPORT void covtosol_vel(const double *P, sol_t *sol);
    EXPORT void covtosol_att(const double *P, sol_t *sol);
    EXPORT void covtosol_bga(const double *Pbg, const double *Pba, sol_t *sol);
...
#define traceb(level, ...) ((void)0)
    ...
    EXPORT void outsol_general(int type, rtk_t *rtk, const obsd_t *obs, const nav_t *nav, const int nu, int mode);
    ...
    EXPORT int amb_BIE(int nb, int num_candidate, const double *y, const double *Qb,  const double *b, double *b_BIE);
    ...
    EXPORT void ins2gnss(const prcopt_t *popt, ins_t *ins, double *pv_g, int n);
    EXPORT void insfix2gnss(ins_t *ins, double *pv_g, int n);
    EXPORT void gnss2ins(rtk_t *rtk, double *pv_g, double *pv_i, int mode);
    EXPORT void earth_update(const prcopt_t *popt,const double *pos, const double *vel, eth_t *eth);
