                  ...
                  char *msg)
{
    ...
    if (nv>nx&&vv>chisqr[nv-nx-1]) {
        ...
        trace(7,"Warning(spp): large chi-square error nv=%d vv=%.1f cs=%.1f\n",nv,vv,chisqr[nv-nx-1]);
    ...
    if (dop[0]<=0.0||dop[0]>MAX_GDOP) {
        sprintf(msg,"gdop error nv=%d gdop=%.1f\n",nv,dop[0]);
...
static void udstate_spp(rtk_t *rtk, const obsd_t *obs, int n, const nav_t *nav)
{
    trace(3,"udstate_spp: n=%d\n",n);
...
{
    double x[NX]={0},dx[NX],Q[NX*NX],*v,*H,*var,sig,*v_,*H_,*var_,mean=0.0,std=0.0,thres=2.0;
    int i,j,k,m,info,stat,nv,ns,*sati;
    
    trace(8,"estpos  : n=%d\n",n);
    
    v=mat(n+5,1); H=mat(NX,n+5); var=mat(n+5,1);
    v_=mat(n+5,1); H_=mat(NX,n+5); var_=mat(n+5,1);
    sati=imat(n+5,1);
    ...
    for (i=0;i<MAXITR;i++) {
        ...
        nv=rescode(i,obs,n,rs,dts,vare,svh,nav,x,opt,ssat,v,H,var,azel,vsat,resp,&ns,sati); 
        
        /* trace(12,"H=\n"); tracemat(12,H,n+5,NX,9,4,0); */
        
        /* outlier detection based on standard normal distribution */
        if (i>=4) {
            mean=calexp(v,nv);
            std=calstd(v,nv);
            m=0;
            ...
            for (j=0;j<nv;j++) {               
                if (fabs(v[j]-mean)/std<thres) {
                    ...
                    trace(7,"iteration(%2d), outlier rejected(spp) sat=%s, res=%13.4f, constructed=%10.4f, thres=%5.2f, el=%4.1f\n",i+1,ssat[sati[j]-1].id,
        ...
        if (nv<NX) {
            sprintf(msg,"lack of valid sats ns=%d\n",nv);
        ...
        if (norm(dx,NX)<1E-4) {
            ...
            free(v); free(H); free(var); free(v_); free(H_); free(var_);
            return stat;
        }
    }
    if (i>=MAXITR) {
        sprintf(msg,"Iteration over limit i=%d!\n",i);
                  ...
                  char *msg)
{
    ...
    if (ssat) {
        ...
        for (i=0;i<n;i++)
            sys=satsys(obs[i].sat,NULL); fr=sys2freid(sys,0,opt);
