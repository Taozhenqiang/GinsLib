                   ...
                   double *rs)
{
    ...
    /* TEME (true equator, mean eqinox) -> ECEF (ref [2] C-2, Appendix C) */
    ...
    matmul("TN",3,3,1,R3,rs_tle  ,rs_pef  ,1.0,0.0);
    matmul("TN",3,3,1,R3,rs_tle+3,rs_pef+3,1.0,0.0);
    rs_pef[3]+=OMGE*rs_pef[1];
    rs_pef[4]-=OMGE*rs_pef[0];
    matmul("NN",3,3,3,R1,R2,W,1.0,0.0);
    matmul("TN",3,3,1,W,rs_pef  ,rs  ,1.0,0.0);
    matmul("TN",3,3,1,W,rs_pef+3,rs+3,1.0,0.0);
    return 1;
}
