...
extern void covtosol(const double *P, sol_t *sol)
{
...
extern void soltocov_vel(const sol_t *sol, double *P)
{
    P[0]     =sol->qv[0]; /* xx */
    P[4]     =sol->qv[1]; /* yy */
    P[8]     =sol->qv[2]; /* zz */
...
extern void covtosol_vel(const double *P, sol_t *sol)
{
                   ...
                   const solopt_t *opt, const prcopt_t *popt)
{
    ...
    if (GINS_LC==popt->GI_mode||GINS_TC==popt->GI_mode||GINS_STC==popt->GI_mode) {
        if (opt->outatt) { /* output attitude */
            p+=sprintf(p,"%s%10.5f%s%10.5f%s%10.5f%s%9.5f%s%8.5f%s%8.5f%s%8.5f%s"
                    "%8.5f%s%8.5f",
                    sep,sol->att[0],sep,sol->att[1],sep,sol->att[2],sep,
                    SQRT(sol->qa[0]),sep,SQRT(sol->qa[1]),sep,SQRT(sol->qa[2]),
                    sep,sqvar(sol->qa[3]),sep,sqvar(sol->qa[4]),sep,
                    sqvar(sol->qa[5]));
        } 
        if (opt->outbga) { /* output imu bg and ba */
            p+=sprintf(p,"%s%12.3f%s%12.3f%s%12.3f%s%12.3f%s%12.3f%s%12.3f%s%12.3f%s%12.3f%s%12.3f%s%12.3f%s%12.3f%s%12.3f",
                    sep,sol->bg[0],sep,sol->bg[1],sep,sol->bg[2],sep,
                  ...
                  const solopt_t *opt, const prcopt_t *popt)
{
    ...
    if (GINS_LC==popt->GI_mode||GINS_TC==popt->GI_mode||GINS_STC==popt->GI_mode) {
        ...
        if (opt->outbga) { /* output imu bg and ba */
            p+=sprintf(p,"%s%10.5f%s%10.5f%s%10.5f%s%10.5f%s%10.5f%s%10.5f%s%10.5f%s%10.5f%s%10.5f%s%10.5f%s%10.5f%s%10.5f",
                    sep,sol->bg[0],sep,sol->bg[1],sep,sol->bg[2],sep,
                    sol->ba[0],sep,sol->ba[1],sep,sol->ba[2],sep,sqvar(sol->qb[0]),
                    sep,sqvar(sol->qb[1]),sep,sqvar(sol->qb[2]),sep,sqvar(sol->qb[3]),
                    sep,sqvar(sol->qb[4]),sep,sqvar(sol->qb[5]));
...
{
    double pos[3],rr[3],enu[3],vel[3],P[9],Q[9];
    int i;
    const char *sep=opt2sep(opt);
    char *p=(char *)buff;    
...
* return : number of output bytes
*-----------------------------------------------------------------------------*/
extern int outsolheads(uint8_t *buff, const prcopt_t *popt, const solopt_t *opt)
{
...
* return : none
*-----------------------------------------------------------------------------*/
extern void outsolhead(FILE *fp, const prcopt_t *popt, const solopt_t *sopt)
{
    uint8_t buff[MAXSOLMSG+1];
    int n;
    ...
    if ((n=outsolheads(buff,popt,sopt))>0) {
        fwrite(buff,n,1,fp);
    }
}
/* output solution body --------------------------------------------------------
