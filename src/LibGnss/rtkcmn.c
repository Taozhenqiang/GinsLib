...
static char sysfre[MAXSYS][MAXFREQ][5]={
...
/* multiply matrix -----------------------------------------------------------*/
extern void matmul(const char *tr,int r1,int c1,int c2,
    ...
    for (i=0;i<r1;i++) for (j=0;j<c2;j++) {
        d=0.0;
        switch (f) {
            case 0: for (k=0;k<c1;k++) d+=A[i*c1+k]*B[k*c2+j]; /* NN */
            break;
            case 1: for (k=0;k<c1;k++) d+=A[i*c1+k]*B[j*c1+k]; /* NT */
            break;
            case 2: for (k=0;k<c1;k++) d+=A[k*r1+i]*B[k*c2+j]; /* TN */
            break;
            case 3: for (k=0;k<c1;k++) d+=A[k*r1+i]*B[j*c1+k]; /* TT */
            break;
         }
         if (beta==0.0) C[j+i*c2]=alpha*d; else C[j+i*c2]=alpha*d+beta*C[j+i*c2];       
    }    
}
...
extern void pos2ecef2(const double *pos,double *r,double *T) {
    double Rnh;
    double sinp=sin(pos[0]),cosp=cos(pos[0]),sinl=sin(pos[1]),cosl=cos(pos[1]);
    double e2=FE_WGS84*(2.0-FE_WGS84),v=RE_WGS84/sqrt(1.0-e2*sinp*sinp);
    ...
    T[0]=-Rnh*sinp*cosl;  T[1]=-Rnh*cosp*sinl;  T[2]=cosp*cosl;
    T[3]=-Rnh*sinp*sinl;  T[4]= Rnh*cosp*cosl;  T[5]=cosp*sinl;
    T[6]=(v*(1.0-e2)+pos[2])*cosp; T[7]=0.0;    T[8]=sinp;
}
...
extern void ecef2enu(const double *pos,const double *r,double *e) {
    ...
    xyz2enu(pos,E);
    matmul("NN",3,3,1,E,r,e,1.0,0.0); /* rn=Cne*re */
...
extern void enu2ecef(const double *pos,const double *e,double *r) {
    ...
    xyz2enu(pos,E);
    matmul("TN",3,3,1,E,e,r,1.0,0.0); /* re=Cne'*rn */
}
...
extern void covenu(const double *pos,const double *P,double *Q) {
    ...
    xyz2enu(pos,E);
    matmul("NN",3,3,3,E,P,EP,1.0,0.0);
    matmul("NT",3,3,3,EP,E,Q,1.0,0.0);  /* Qn=Cne*Qe*Cne' */
}
...
extern void covecef(const double *pos,const double *Q,double *P) {
    ...
    xyz2enu(pos,E);
    matmul("TN",3,3,3,E,Q,EQ,1.0,0.0);
    matmul("NN",3,3,3,EQ,E,P,1.0,0.0);  /* Qe=Cne'*Qn*Cne */
}
    ...
    while (fgets(buff,sizeof(buff),fp)) {
        ...
        if (erp->n >=erp->nmax) {
            ...
            if (!erp_data) {
                ...
                erp->n=erp->nmax=0;
                fclose(fp);
                trace(1,"erp data memory allocation failure:!\n");
                return 0;
            }
            erp->data=erp_data;
        }
...
extern void sunmoonpos(gtime_t tutc,const double *erpv,double *rsun,
    ...
    if (rsun)
        matmul("NN",3,3,1,U,rs,rsun,1.0,0.0);
    if (rmoon)
        matmul("NN",3,3,1,U,rm,rmoon,1.0,0.0);
    if (gmst)
        *gmst=gmst_;
}
/* mutipath correct-------------------------------------------------------------
