...
static void readant(vt_t *vt, prcopt_t *opt, nav_t *nav)
{
    ...
    if (!*filopt.antp) return;
    
    if (readpcv(filopt.antp,&pcvs,&pcvr)) {
        for (i=0;i<2;i++) {
            opt->pcvr[i]=rpcv0;
            if (!*opt->anttype[i]) continue;
            if (!(rpcv=searchrpcv(opt->anttype[i],time,&pcvr))) {
                vt_printf(vt,"no antenna %s in %s",opt->anttype[i],filopt.antp);
                continue;
            }
            opt->pcvr[i]=*rpcv;
        }
        for (i=0;i<MAXSAT;i++) {
            nav->spcvs[i]=spcv0;
            if (!(spcv=searchspcv(i+1,time,&pcvs))) continue;
            nav->spcvs[i]=*spcv;
        }        
    }
    else vt_printf(vt,"antenna file open error %s",filopt.antp);
...
static int startsvr(vt_t *vt)
{
    ...
    if (*filopt.mgex_dcb) {
        ...
        readdcb(&prcopt,filopt.mgex_dcb,&svr.nav,sta);  
    }
...
static void prstatus(vt_t *vt)
{
    ...
    rtksvrlock(&svr);
    rtk=svr.rtk;
    thread=(pthread_t)svr.thread; /* tzq */
    cycle=svr.cycle;
    state=svr.state;
